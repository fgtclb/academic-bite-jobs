...
:yaml:`fgtclb/academic-bite-jobs` shipped its own :file:`constants.typoscript`
...
*   :file:`Configuration/TypoScript/List/` holds the TypoScript of the
    :guilabel:`Job list` content element and is what the static template
...
:file:`Configuration/page.tsconfig` removes :typoscript:`academicbitejobs_list`
from the selectable content element types, and the page TSconfig of the
component adds it back — so the element is offered where it is wanted instead
of on every page of every installation. The TCA registration itself did not
move, so the frontend renders existing records exactly as before. Editing such
a record in the backend is a different matter — read the warning below before
upgrading.
...
through the page field :guilabel:`Page TSconfig`.

..  warning::

    Do not open an existing :guilabel:`Job list` record in the backend form on a
    page that does not include that page TSconfig. An item removed through
...
The set :yaml:`fgtclb/academic-bite-jobs` keeps its name and keeps delivering
...
..  list-table::
    ...
    *   -   Old entry
        -   New entry
    *   -   :guilabel:`Academic Bite Jobs (academic_bite_jobs)`,
            stored as `EXT:academic_bite_jobs/Configuration/TypoScript`
        -   :guilabel:`Academic Bite Jobs: All components (academic_bite_jobs)`,
            stored as `EXT:academic_bite_jobs/Configuration/TypoScript/Full` —
            or :guilabel:`Academic Bite Jobs: Job list (academic_bite_jobs)`,
            stored as `EXT:academic_bite_jobs/Configuration/TypoScript/List`
...
:guilabel:`Academic Bite Jobs: All components (academic_bite_jobs)`, stored as
`EXT:academic_bite_jobs/Configuration/TSconfig/Full/page.tsconfig`. Without it
the content element is not selectable any more, and existing records of it lose
...
..  list-table::
    ...
    *   -   `EXT:academic_bite_jobs/Configuration/TSconfig/Wizards/NewContentElement.tsconfig`
        -   `EXT:academic_bite_jobs/Configuration/TSconfig/List/page.tsconfig`
    *   -   `EXT:academic_bite_jobs/Configuration/TSconfig/Wizards/*.tsconfig`
...
..  list-table::
    ...
    *   -   `fgtclb/academic-bite-jobs-list`
        -   The :guilabel:`Job list` content element only.

..  index:: TypoScript, TSConfig, Backend, ext:academic_bite_jobs
